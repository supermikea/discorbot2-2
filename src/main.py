...
async def play(ctx, *, url):
	...
	global currently_playing, vc_queue, vc_r_queue
	try:
...
# check if playing audio then play the next one in the queue
@tasks.loop(seconds=1)
async def is_playing_audio(ctx):
	if ctx.voice_client.is_playing_audio():
		# do some shit
	else:
		# do some other shit: remove the file
